...
func NewHydrology(controller *Controller) Hydrology {
	return Hydrology{
		controller: controller,
	}
}

// CalculateHydrologyMap calculates the available water at every point on the terrain. It uses the average rainfall and groundwater from
...
func (h Hydrology) CalculateHydrologyMap(m *Map, edaphicMap [][]float64, soilIdsMap [][]int, imageInsolationMap *Image, biom *Biom) [][]float64 {
	hydrologyMap := new2DArr(len(edaphicMap))
	for y := 0; y < len(edaphicMap); y++ {
		for x := 0; x < len(edaphicMap[y]); x++ {
			depth := edaphicMap[y][x]
			soilId := soilIdsMap[y][x]
			soil := h.controller.SearchSoil(soilId)
			var depthCoefficient float64
			if depth >= 100 {
				depthCoefficient = 1.0
			} else {
				depthCoefficient = depth / 100
			}
			waterSupply := (biom.groundwater + biom.avg_rainfall_per_day) * depthCoefficient * soil.water_absorption
			evaporatedWater := (imageInsolationMap.image[y][x] * K_CALORIES_NEEDED_TO_EVAPORATE_1_G_WATER) / 1000
			if evaporatedWater > waterSupply {
				evaporatedWater = waterSupply
			}
			waterSupply -= evaporatedWater
			hydrologyMap[y][x] = waterSupply // * (map.pixelSize ** 2)
		}
	}
	return hydrologyMap
}
